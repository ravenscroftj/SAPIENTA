...
def getSplitSentences( paperid ):
    ...
    fout, outfile = tempfile.mkstemp()
    s = SentenceSplitter()

    infile = os.path.join(TEST_FILES_PATH, "noSents", paperid + ".xml")

    s.split(infile, outfile, extra_blacklist=['CURRENT_TITLE'])
...
def getSSSplitResult( paperid ):
    ...
    paperName = os.path.join( TEST_FILES_PATH, "sssplitresults", paperid + "_mode2.xml2")
    ...
    for root, dirs, files in os.walk(unsplit):
        ...
        for file in files:
            ...
            if file.endswith(".xml"):
                ...
                sssents = getSplitSentences( paperid )
