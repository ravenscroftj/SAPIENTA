...
highLevelContainerElements = ["DIV", "div", "sec", "section", "abstract", "article-title", "list-item", "region"]
...
class SSSplit:
    ...
    def split(self, filename, outname=None, pp=True, mode2=False, *args, **kwargs):
        ...
        for container in highLevelContainerElements:
            for el in self.root.iterfind(f".//{{*}}{container}"):
    ...
    def split_high_level_container(self, containerEl):
        ...
        for containerType in pLevelContainerElements:
            for el in set(containerEl.findall(f".//{{*}}{containerType}")):
                
                if len(el.findall('.//s')) > 0:
                    continue
