		...
		<html>
			<head>
				...
				<style type="text/css">
					...
					select.type2 option { color: white; }
					select.type2 option.None { color:black; }
					select.type2 option.default { color: #666; }
					select.type3 option { color: white; }
					select.type3 option.None { color:black; }
					select.type3 option.default { color: #666; }
					
					<!-- Here put your preferred colours for your annotation concepts -->
					<!-- Put your own concepts here instead -->
					div.Background, option.Bac 	{ background-color: #666; color: white;}
					div.Conclusion, option.Con	{ background-color: #600; color: white;}
					div.Example, option.Exa		{ background-color: #660; color: white;}
					div.Experiment, option.Exp 	{ background-color: #060; color: white;} 
					div.Goal, option.Goa 		{ background-color: #066; color: white;} 
					div.Hypothesis, option.Hyp	{ background-color: #006; color: white;} 
					div.Method, option.Met 		{ background-color: #606; color: white;} 
					div.Model, option.Mod		{ background-color: #c00; color: white;} 
					div.Motivation, option.Mot  { background-color: #cc0; color: white;} 
					div.Object, option.Obj		{ background-color: #0c0; color: white;} 
					div.Observation, option.Obs	{ background-color: #0cc; color: white;} 
					div.Problem, option.Pro		{ background-color: #00c; color: white;} 
					div.Results, option.Res		{ background-color: #c0c; color: white;}
...
textarea#comment{margin-bottom: 20%;}
					...
					div.dropdowns2{margin-left:6%;}				
					div.dropdowns3{margin-left:6%;}
					div.sid {
						text-align:right;
						float:left; 
						padding-right:1%; 
			...
			<body onload="initialise()"> <!-- onload= "getOptions()"> -->
				...
				<div class="mainpage" id="mainpage">
					...
					<p>
						...
						|
						<xsl:element name="a">
							<xsl:attribute name="href">ART?action=classify&amp;name=<xsl:value-of
													select="//mode2/@name" />&amp;mode=2
     						</xsl:attribute>
							Classify
						</xsl:element>												
						...
						|
					<!-- this is a dropdown to allow the display of only one type of category, so that conceptIDs can changed more easily -->	
					<select class="type" onchange="showType(this)">
					...
					<p>
						<div class="Background topkey">Background</div>
						<div class="Conclusion topkey">Conclusion</div>
						<!-- Removing Example -->
						<!--<div class="Example topkey">Example</div>-->
						<div class="Experiment topkey">Experiment</div>
						<div class="Goal topkey">Goal</div>
						<div class="Hypothesis topkey">Hypothesis</div>
						<div class="Method topkey">Method</div>
						<div class="Model topkey">Model</div>
						<div class="Motivation topkey">Motivation</div>
						<div class="Object topkey">Object</div>
						<div class="Observation topkey">Observation</div>
						
						<!-- Removing Problem -->
						<!-- <div class="Problem topkey">Problem</div>-->
						<div class="Results topkey">Results</div>
					</p>
					
					</div>
					<!-- SciXML papers -->
					<div class="allsentences">
						<xsl:for-each select="PAPER|//article">
	...
	<xsl:template match="PAPER" name="PAPER">
		...
		<xsl:for-each select= "//PAPER/article/front/article-meta/title-group/article-title|//CURRENT_TITLE|//TITLE"><!--  "/PAPER/TITLE|/PAPER/CURRENT_TITLE|/PAPER/article/TITLE|/PAPER/article/front/article-meta/title-group/TITLE|//TITLE|//article-title"> --> 
			<xsl:choose>
		...
		<!--  -->
		<xsl:for-each select = "//abstract|//ABSTRACT"> <!-- //*[translate('abstract', 'abstract','ABSTRACT')] -->
			<!--<xsl:choose>
         		<xsl:when test="position() = 1">
					<hr /> -->
						<p>
		...
		<xsl:for-each select="//body|//BODY"><!-- BODY/DIV"> --> <!-- 	BODY to //BODY -->
			<xsl:apply-templates />
		</xsl:for-each>	
	...
	<xsl:variable name="drop2prefix">dp2</xsl:variable>
	<xsl:variable name="drop3prefix">dp3</xsl:variable>
	...
	<xsl:template name="s" match="s">
		...
		<div name="sentenceDiv" class="sentencearea">
			...
			<div name="drop1" class="dropdowns">
			    <xsl:attribute name="sid"><xsl:value-of select="@sid"/></xsl:attribute>
				<xsl:choose>
				<!-- When a sentence contains annotations -->
				<!-- Put your own schema name here instead of "CoreSc1"-->
				<xsl:when test="CoreSc1">
					<select class="type" onchange="changeType(this,true,1)">
						<xsl:attribute name="sid"><xsl:value-of select="@sid"/></xsl:attribute>
						...
						<!--  Put your own set of concepts in here in place of "None", "Bac" etc. -->
						<!-- Removing "None" -->
						<!-- <option class="None" value="None"><xsl:if test="CoreSc1[@type = 'None']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>None</option>-->
						<option class="Bac" value="Bac" ><xsl:if test="CoreSc1[@type = 'Bac']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Background</option>
						<option class="Con" value="Con" ><xsl:if test="CoreSc1[@type = 'Con']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Conclusion</option>
							<!-- Removing "Example" -->
						<!-- <option class="Exa" value="Exa" ><xsl:if test="CoreSc1[@type = 'Exa']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Example</option>-->
						<option class="Exp" value="Exp" ><xsl:if test="CoreSc1[@type = 'Exp']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Experiment</option>
						<option class="Goa" value="Goa" ><xsl:if test="CoreSc1[@type = 'Goa']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Goal</option>
						<option class="Hyp" value="Hyp" ><xsl:if test="CoreSc1[@type = 'Hyp']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Hypothesis</option>
						<option class="Met" value="Met">
							<xsl:if test="CoreSc1[@type = 'Met']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Method</option>
						<option class="Mod" value="Mod" ><xsl:if test="CoreSc1[@type = 'Mod']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Model</option>
						<option class="Mot" value="Mot" ><xsl:if test="CoreSc1[@type = 'Mot']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Motivation</option>
						<option class="Obj" value="Obj" ><xsl:if test="CoreSc1[@type = 'Obj']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Object</option>
						<option class="Obs" value="Obs" ><xsl:if test="CoreSc1[@type = 'Obs']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Observation</option>
							<!-- Removing "Problem" -->
						<!--  <option class="Pro" value="Pro" ><xsl:if test="CoreSc1[@type = 'Pro']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Problem</option>-->
						<option class="Res" value="Res" ><xsl:if test="CoreSc1[@type = 'Res']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Result</option>	
					</select>
					<!-- if you are making use of subtypes, put your own subtype tests in the following "choose" block -->
					<select class="subtype" onchange="changeSubType(this,1)">
						<xsl:attribute name="sid"><xsl:value-of select="@sid"/></xsl:attribute>
						<option class="default" value="default">
								<xsl:attribute name="selected">selected</xsl:attribute>
						</option>
						<xsl:variable name="sub">
							<xsl:choose>
								<xsl:when test="CoreSc1[@advantage = 'Yes']">Advantage</xsl:when>
								<xsl:when test="CoreSc1[@advantage = 'No']">Disadvantage</xsl:when>																	
								<xsl:when test="CoreSc1[@novelty = 'Old']">Old</xsl:when>
								<xsl:when test="CoreSc1[@novelty = 'New']">New</xsl:when>
                                <xsl:when test="CoreSc1[@novelty = 'Future']">Future</xsl:when>
					...
					<select class="conceptid" onchange="selectConceptID(this,1)">
						<xsl:attribute name="sid"><xsl:value-of select="@sid"/></xsl:attribute>
						<option class="default" value="default">
								<xsl:attribute name="selected">selected</xsl:attribute>
						</option>
						<option>
							<xsl:attribute name="class"><xsl:value-of select="CoreSc1/@conceptID"/></xsl:attribute>
							<xsl:attribute name="value"><xsl:value-of select="CoreSc1/@conceptID"/></xsl:attribute>
							<xsl:attribute name="selected">selected</xsl:attribute>
							<xsl:value-of select="CoreSc1/@conceptID"/>
						</option>
					</select>
				</xsl:when>
				...
				<xsl:otherwise>				
					<select class="type" onchange="changeType(this,true,1)">
						...
						<option class="Con" value="Con" >Conclusion</option>
						<!-- Removing "Example" -->
						<!-- <option class="Exa" value="Exa" >Example</option>-->
						<option class="Exp" value="Exp" >Experiment</option>
						<option class="Goa" value="Goa" >Goal</option>
						<option class="Hyp" value="Hyp" >Hypothesis</option>
						<option class="Met" value="Met" >Method</option>
						...
						<option class="Obs" value="Obs" >Observation</option>
						<!-- Removing "Problem" -->
						<!-- <option class="Pro" value="Pro" >Problem</option>-->
						<option class="Res" value="Res" >Result</option>	
					</select>
			...
			<xsl:choose>
			<xsl:when test="CoreSc2">
			<xsl:choose>
			<xsl:when test="CoreSc3">
				<button type="button" name ="multiButton" onclick="(this)" onmouseover="showToolTip()"><xsl:attribute name="id"><xsl:value-of select="@sid"/></xsl:attribute><xsl:attribute name="flag1"><xsl:value-of select="1"/></xsl:attribute><xsl:attribute name="flag2"><xsl:value-of select="1"/></xsl:attribute>Multi Annotation</button>
			</xsl:when>
			<xsl:otherwise>
				<button type="button" name ="multiButton" onclick="addDropDown(this)" onmouseover="showToolTip()"><xsl:attribute name="id"><xsl:value-of select="@sid"/></xsl:attribute><xsl:attribute name="flag1"><xsl:value-of select="1"/></xsl:attribute><xsl:attribute name="flag2"><xsl:value-of select="0"/></xsl:attribute>Multi Annotation</button>
			</xsl:otherwise>
			</xsl:choose>
			</xsl:when>
			<xsl:otherwise>
				<button type="button" name ="multiButton" onclick="addDropDown(this)" onmouseover="showToolTip()"><xsl:attribute name="id"><xsl:value-of select="@sid"/></xsl:attribute><xsl:attribute name="flag1"><xsl:value-of select="0"/></xsl:attribute><xsl:attribute name="flag2"><xsl:value-of select="0"/></xsl:attribute>Multi Annotation</button>
			</xsl:otherwise>
			</xsl:choose>
			</div> <!-- end of dropdowns 1-->
			<div id="{concat($drop2prefix,@sid)}" name="drop2" class="dropdowns2"> <!-- style="position:absolute;left:-4000px;"  -->
				<xsl:attribute name="sid"><xsl:value-of select="@sid"/></xsl:attribute>
				<xsl:choose>
				<!-- When a sentence contains annotations -->
				<!-- Put your own schema name here instead of "CoreSc1"-->
				<xsl:when test="CoreSc2">
					<select class="type2" onchange="changeType(this,true,2)">
						<xsl:attribute name="sid"><xsl:value-of select="@sid"/></xsl:attribute>
						...
						<!--  Put your own set of concepts in here in place of "None", "Bac" etc. -->
						<!-- Removing "None" -->
						<!-- <option class="None" value="None"><xsl:if test="CoreSc2[@type = 'None']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>None</option>-->
						<option class="Bac" value="Bac" ><xsl:if test="CoreSc2[@type = 'Bac']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Background</option>
						<option class="Con" value="Con" ><xsl:if test="CoreSc2[@type = 'Con']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Conclusion</option>
							<!-- Removing "Example" -->
						<!-- <option class="Exa" value="Exa" ><xsl:if test="CoreSc2[@type = 'Exa']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Example</option>-->
						<option class="Exp" value="Exp" ><xsl:if test="CoreSc2[@type = 'Exp']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Experiment</option>
						<option class="Goa" value="Goa" ><xsl:if test="CoreSc2[@type = 'Goa']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Goal</option>
						<option class="Hyp" value="Hyp" ><xsl:if test="CoreSc2[@type = 'Hyp']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Hypothesis</option>
						<option class="Met" value="Met">
							<xsl:if test="CoreSc2[@type = 'Met']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Method</option>
						<option class="Mod" value="Mod" ><xsl:if test="CoreSc2[@type = 'Mod']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Model</option>
						<option class="Mot" value="Mot" ><xsl:if test="CoreSc2[@type = 'Mot']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Motivation</option>
						<option class="Obj" value="Obj" ><xsl:if test="CoreSc2[@type = 'Obj']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Object</option>
						<option class="Obs" value="Obs" ><xsl:if test="CoreSc2[@type = 'Obs']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Observation</option>
							<!-- Removing "Problem" -->
						<!--  <option class="Pro" value="Pro" ><xsl:if test="CoreSc2[@type = 'Pro']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Problem</option>-->
						<option class="Res" value="Res" ><xsl:if test="CoreSc2[@type = 'Res']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Result</option>	
					</select>
					<!-- if you are making use of subtypes, put your own subtype tests in the following "choose" block -->
					<select class="subtype2" onchange="changeSubType(this,2)">
						<xsl:attribute name="sid"><xsl:value-of select="@sid"/></xsl:attribute>
						<option class="default" value="default">
								<xsl:attribute name="selected">selected</xsl:attribute>
						</option>
						<xsl:variable name="sub2">
							<xsl:choose>
								<xsl:when test="CoreSc2[@advantage = 'Yes']">Advantage</xsl:when>
								<xsl:when test="CoreSc2[@advantage = 'No']">Disadvantage</xsl:when>																	
								<xsl:when test="CoreSc2[@novelty = 'Old']">Old</xsl:when>
								<xsl:when test="CoreSc2[@novelty = 'New']">New</xsl:when>
                                <xsl:when test="CoreSc2[@novelty = 'Future']">Future</xsl:when>
								<xsl:otherwise>None</xsl:otherwise>
							</xsl:choose>
						</xsl:variable>
						<xsl:if test="$sub2 != 'None'">
							...
							<option>
								<xsl:attribute name="class"><xsl:copy-of select="$sub2"/></xsl:attribute>
								<xsl:attribute name="value"><xsl:copy-of select="$sub2"/></xsl:attribute>
								<xsl:attribute name="selected">selected</xsl:attribute>
								<xsl:copy-of select="$sub2"/>
							</option>
						</xsl:if>
					</select>
					<!-- If you have concepts which do not require concept IDs, leave this unchanged - otherwise put the name of your own schema -->
					<select class="conceptid2" onchange="selectConceptID(this,2)">
						<xsl:attribute name="sid"><xsl:value-of select="@sid"/></xsl:attribute>
						<option class="default" value="default">
								<xsl:attribute name="selected">selected</xsl:attribute>
						</option>
						<option>
							<xsl:attribute name="class"><xsl:value-of select="CoreSc2/@conceptID"/></xsl:attribute>
							<xsl:attribute name="value"><xsl:value-of select="CoreSc2/@conceptID"/></xsl:attribute>
							<xsl:attribute name="selected">selected</xsl:attribute>
							<xsl:value-of select="CoreSc2/@conceptID"/>
						</option>
					</select>
				</xsl:when>
			</xsl:choose>
			<!--<input type="submit" value="Add" />-->
			<!-- <button id="multi" type="button" onclick="addDropDown()" onmouseover="showToolTip()">Multi Annotation</button> -->
			</div>
	<div id="{concat($drop3prefix,@sid)}" name="drop3" class="dropdowns3">
				<xsl:attribute name="sid"><xsl:value-of select="@sid"/></xsl:attribute>
				<xsl:choose>
				<!-- When a sentence contains annotations -->
				<!-- Put your own schema name here instead of "CoreSc1"-->
				<xsl:when test="CoreSc3">
					<select class="type3" onchange="changeType(this,true,3)">
						<xsl:attribute name="sid"><xsl:value-of select="@sid"/></xsl:attribute>
						...
						<!--  Put your own set of concepts in here in place of "None", "Bac" etc. -->
						<!-- Removing "None" -->
						<!-- <option class="None" value="None"><xsl:if test="CoreSc3[@type = 'None']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>None</option>-->
						<option class="Bac" value="Bac" ><xsl:if test="CoreSc3[@type = 'Bac']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Background</option>
						<option class="Con" value="Con" ><xsl:if test="CoreSc3[@type = 'Con']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Conclusion</option>
							<!-- Removing "Example" -->
						<!-- <option class="Exa" value="Exa" ><xsl:if test="CoreSc3[@type = 'Exa']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Example</option>-->
						<option class="Exp" value="Exp" ><xsl:if test="CoreSc3[@type = 'Exp']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Experiment</option>
						<option class="Goa" value="Goa" ><xsl:if test="CoreSc3[@type = 'Goa']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Goal</option>
						<option class="Hyp" value="Hyp" ><xsl:if test="CoreSc3[@type = 'Hyp']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Hypothesis</option>
						<option class="Met" value="Met">
							<xsl:if test="CoreSc3[@type = 'Met']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Method</option>
						<option class="Mod" value="Mod" ><xsl:if test="CoreSc3[@type = 'Mod']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Model</option>
						<option class="Mot" value="Mot" ><xsl:if test="CoreSc3[@type = 'Mot']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Motivation</option>
						<option class="Obj" value="Obj" ><xsl:if test="CoreSc3[@type = 'Obj']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Object</option>
						<option class="Obs" value="Obs" ><xsl:if test="CoreSc3[@type = 'Obs']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Observation</option>
							<!-- Removing "Problem" -->
						<!--  <option class="Pro" value="Pro" ><xsl:if test="CoreSc3[@type = 'Pro']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Problem</option>-->
						<option class="Res" value="Res" ><xsl:if test="CoreSc3[@type = 'Res']">
								<xsl:attribute name="selected">selected</xsl:attribute>
							</xsl:if>Result</option>	
					</select>
					<!-- if you are making use of subtypes, put your own subtype tests in the following "choose" block -->
					<select class="subtype3" onchange="changeSubType(this,3)">
						<xsl:attribute name="sid"><xsl:value-of select="@sid"/></xsl:attribute>
						<option class="default" value="default">
								<xsl:attribute name="selected">selected</xsl:attribute>
						</option>
						<xsl:variable name="sub3">
							<xsl:choose>
								<xsl:when test="CoreSc3[@advantage = 'Yes']">Advantage</xsl:when>
								<xsl:when test="CoreSc3[@advantage = 'No']">Disadvantage</xsl:when>																	
								<xsl:when test="CoreSc3[@novelty = 'Old']">Old</xsl:when>
								<xsl:when test="CoreSc3[@novelty = 'New']">New</xsl:when>
                                <xsl:when test="CoreSc3[@novelty = 'Future']">Future</xsl:when>
								<xsl:otherwise>None</xsl:otherwise>
							</xsl:choose>
						</xsl:variable>
						<xsl:if test="$sub3 != 'None'">
							...
							<option>
								<xsl:attribute name="class"><xsl:copy-of select="$sub3"/></xsl:attribute>
								<xsl:attribute name="value"><xsl:copy-of select="$sub3"/></xsl:attribute>
								<xsl:attribute name="selected">selected</xsl:attribute>
								<xsl:copy-of select="$sub3"/>
							</option>
						</xsl:if>
					</select>
					<!-- If you have concepts which do not require concept IDs, leave this unchanged - otherwise put the name of your own schema -->
					<select class="conceptid3" onchange="selectConceptID(this,3)">
						<xsl:attribute name="sid"><xsl:value-of select="@sid"/></xsl:attribute>
						<option class="default" value="default">
								<xsl:attribute name="selected">selected</xsl:attribute>
						</option>
						<option>
							<xsl:attribute name="class"><xsl:value-of select="CoreSc3/@conceptID"/></xsl:attribute>
							<xsl:attribute name="value"><xsl:value-of select="CoreSc3/@conceptID"/></xsl:attribute>
							<xsl:attribute name="selected">selected</xsl:attribute>
							<xsl:value-of select="CoreSc3/@conceptID"/>
						</option>
					</select>
				</xsl:when>
			</xsl:choose>
	...
	<xsl:template match="DIV/HEADER|/PAPER/article/SEC/title" priority="-5">
		<h2>
			<xsl:apply-templates />
		</h2>
	</xsl:template>
 ...
 <!-- Addition for case where REF has REFID -->
    <xsl:template match="REF[@REFID]">
		<xsl:choose>
			<xsl:when test="substring-before(@REFID, ' ')">
				<a>
					<xsl:attribute name="href">#ref_<xsl:value-of
							select="substring-before(@REFID, ' ')" />
	  </xsl:attribute>
					<xsl:attribute name="title">
	    <xsl:call-template name="citations">
	      <xsl:with-param name="cits" select="@REFID" />
	    </xsl:call-template>
	  </xsl:attribute>
	                 <sup class="ref1">
						<xsl:choose>
